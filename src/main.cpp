...
struct Renderer {
    ...
    void render(const Particles& particles, const glm::mat4& projection) {
        ...
        glUniform2f(glGetUniformLocation(shaderProgram, "uParticleSize"), 0.02f, 0.02f);
...
int main() {
    ...
    Particles particles;
    particles.spawnGrid(10, 10, 0.05f, -0.5f, -0.5f);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (currentTime - lastTime >= 1.0) {
            std::cout << "FPS: " << frameCount << " | Sim Time: " << simTime << "s" << std::endl;
            frameCount = 0;
